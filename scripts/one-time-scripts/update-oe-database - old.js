...
/**
 * 查询行是否存在于数据库
 * 使用 (oe_number, line_number, job_number) 组合作为唯一标识
 * 这样不依赖尚未创建的PO号，可以在生成PO前就找到现有记录
 */
function findOrderItem(db, oeNumber, lineNumber, jobNumber) {
  let query = `
  ...
  const params = [jobNumber, lineNumber];
...
/**
 * 规范化PO号
 * 规则: 移除空格、转大写、Rev.→R.、确保R.前有破折号、-RN→-R.N、R.0N→R.N
 * 
 * @param {string} poNumber - 原始PO号
 * @returns {string} 规范化后的PO号
 */
function normalizePONumber(poNumber) {
  if (!poNumber) return '';
  
  let normalized = String(poNumber).replace(/\s+/g, '');                    // 移除所有空格
  normalized = normalized.toUpperCase();                                     // 转大写
  normalized = normalized.replace(/REV\./g, 'R.');                          // Rev.→R.
  normalized = normalized.replace(/([A-Z0-9])R\./g, '$1-R.');              // 确保R.前有破折号
  normalized = normalized.replace(/-R(\d)(?!\d)/g, '-R.$1');                // -RN→-R.N (仅1位数字，负向先行)
  normalized = normalized.replace(/R\.0(\d)/g, 'R.$1');                     // R.0N→R.N (移除前导零)
  normalized = normalized.replace(/\s*-\s*/g, '-');                         // 清理破折号周围的空格
...
function insertNewOrderItem(db, rowData) {
  try {
    ...
    if (!poNumber || poNumber.toLowerCase() === 'npo') {
      // 使用固定格式：NPO-{oe_number}
      // 一个oe_number对应一个PO，一个PO可以有多个job和order_item
      poNumber = `NPO-${rowData.oe_number}`;
    }
  ...
  try {
    ...
    const transaction = db.transaction(() => {
      ...
      for (let i = 0; i < excelRows.length; i++) {
        ...
        try {
          // 查找是否存在
          const existing = findOrderItem(db, row.oe_number, row.line_number, row.job_number);
