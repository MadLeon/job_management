 ...
 * 
 * 面包屑导航:
 * - Purchase Orders > PO Number > Job Number > Part Number（若为装配体）或 Drawing Number（若为图纸）
 * 
 * @component
...
export default function DrawingDetailPage() {
  ...
  return (
    <Stack spacing={3} sx={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
      <Breadcrumb
        locationLayer={['purchase orders', currentJob?.po_number || 'Loading', job_number || 'Loading...', breadcrumbLabel || 'Detail']}
        href={["/purchase-orders", `/purchase-orders/${currentJob?.po_number}`, `/active-jobs/${job_number}`, '#']}
      />
